...
/* Euclidea */
double Euclidean(double *x, double *y, int minSize) {

    double d = 0;
    int min = minSize;//(sizeof(x) < sizeof(y)) ? sizeof(x) : sizeof(y);
...
double Dist(double *a, double *b, int minSize) {
//     if(DistanceMeasureUsed=="Max")
//       return Max3D(a,b);
    if(DistanceMeasureUsed=="Euclidean")
         return Euclidean(a,b,minSize);   
...
//DTWMatching DTWClassic(vector< vector <double> > s, int sIni, int sEnd, vector< vector <double> > t, int tIni, int tEnd) {
 DTWMatching DTWClassic( double  **s, int sIni, int sEnd, double **t, int tIni, int tEnd) {
    ...
    for (int i = sIni; i <= sEnd; i++)
        for (int j = tIni; j <= tEnd; j++){            
            cost[i - sIni][j - tIni] = Dist(s[i], t[j],Num_Angles+1);            
        }
