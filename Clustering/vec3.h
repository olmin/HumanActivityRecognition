...
/*! Angle of two vectors. Added to vec3.h by Olmo */ 
inline float angle2vectors(const vec3& v1, const vec3& v2){
      vec3 c = crossprod(v1, v2);    
      return std::atan2(c.Length(), dotprod(v1, v2));          
  }

/*! Signed angle of two vectors compared to a third vector as reference. Added to vec3.h by Olmo */ 
inline float signed_angle(const vec3& v1, const vec3& v2, const vec3& reference){
      vec3 c = crossprod(v1, v2);    
      float angle = std::atan2(c.Length(), dotprod(v1, v2));    
      return dotprod(c, reference) < 0.f ? -angle : angle;
  }

inline double euclidean(vec3 a, vec3 b) {
